...
public class ProofChecker {
    ...
    public ProofChecker (String [ ] args) {
    	...
    	//if two args , take it from a file in args [1]
        if (args.length > 1) {
            lines = new InputSource (args[1]);
        } else {
        	//otherwise take it from keyboard
            lines = new InputSource ( );
        }
        //initalize theorem set
        myTheorems = new TheoremSet ( );
        //if there are args
        if (args.length > 0) {
        	//make input source to make theoremset
        	//format: "name expression"
            InputSource theoremsIn = new InputSource (args[0]);
            //read until there's no more stuff
            while (true) {
                ...
                String theoremName = thmScanner.next ( ); //read the line without whitespace
                Expression theorem = null;
                //paired input name, theorem
                try {
                    theorem = new Expression (thmScanner.next ( ));
                    System.out.print(theorem.toString());
                    myTheorems.put (theoremName, theorem);//WTF!!!?!!??
                } catch (Exception e) {
                	//catch theorem errors
